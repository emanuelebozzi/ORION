...
geometry_csv_path = '/home/emanuele/post_doc/ORION/safe/porotomo/POROTOMO_GEOM_converted.txt'
simplified_geometry_csv = '/home/emanuele/post_doc/ORION/safe/porotomo/simplified_geometry_porotomo.csv'

#directory containing .mseed and .h5 files


#das_directory = '/home/emanuele/post_doc/codici/kefalonia/input_detected_events'
das_directory = '/home/emanuele/post_doc/ORION/safe/porotomo'
...
npy_pattern = os.path.join(das_directory, 'POROTOMO_20161403_104157_104257_downsampled.npy')      # for generic .npy files
...
clustering_save_path = '/home/emanuele/post_doc/ORION/safe/clustering_results_porotomo.pkl'




def main():
    output_root = '/home/emanuele/post_doc/ORION/safe/output_orion_porotomo'
    for i, (name, das_data_path) in enumerate(tqdm(names_and_paths, desc="Processing datasets")):
        ...
        if i == 0:
            steps = [
                ...
                ("Compute average azimuth", lambda: selector.compute_average_azimuth(chunk_size=5)), #chunk size in number of channels to average the azimuth 
                ("Estimate DBSCAN eps", lambda: setattr(
                    selector, 'eps_est', selector.estimate_dbscan_eps(min_samples=5)
                )),  # minimum number of samples to define a family

                ("Cluster by azimuth distance", lambda: selector.cluster_by_azimuth_distance(
                    eps=0.3,   # use estimated eps instead of fixed 0.5
                    min_samples=5,
                    window_size=5
                )),
        ...
        selector.analyze_waveforms_and_select_best(
            gauge_length = 10,    
            subsection_size=int(selector.avg_cluster_size),  #minimum number of channels to define a subsection
            win_snr=4,            
            step_snr=1, 
            win=8,  #for coherece 
            start_event=None,
            ...
            noise_rms_weight = 1,
            n_final_select=50,     # <<< NEW: select top 10 traces only
